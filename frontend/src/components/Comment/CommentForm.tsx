...
export default function CommentForm({ onSubmit, initialValues }: CommentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!isValid || isSubmitting) return; // Prevent invalid or duplicate submits

    setIsSubmitting(true);
    setError(null);

    try {
      // Call async submit prop with trimmed data
      await onSubmit({
      ...
      >
        ...
        <div className="relative">
          <input
            ...
            required
          />
          <label className="absolute left-2 top-1 text-[8px] text-customGrayLabel peer-placeholder-shown:top-2 peer-placeholder-shown:text-xs peer-placeholder-shown:text-customGrayLabelPlaceholder transition-all pointer-events-none">
            Your name
          </label>
        </div>
        ...
        <div className="relative flex items-end gap-2">
          <div className="relative w-full">
            <textarea
              ...
              required
            />
            <label className="absolute left-2 top-1 text-[10px] text-customGrayLabel peer-placeholder-shown:top-2 peer-placeholder-shown:text-xs peer-placeholder-shown:text-customGrayLabelPlaceholder transition-all pointer-events-none">
